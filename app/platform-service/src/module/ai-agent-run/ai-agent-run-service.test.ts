...
test("get: given repository fails, when called, then returns err", async () => {
  // given
...
test("list: given no filter, when called, then forwards undefined to repository", async () => {
  // given
  mockRepository.findMany.mockResolvedValue(ok({ data: [] }));

  // when
  await aiAgentRunService.list({ ctx });
...
test("list: given repository fails, when listed, then returns err", async () => {
  ...
  // when
  const result = await aiAgentRunService.list({ ctx });
...
test("update: given an existing run, when updated, then merges the patch with current data and forwards expectedVersion", async () => {
  // given
  const existing = makeRun();
  const updated = makeRun({ status: "stopped", _version: 2 });
...
test("update: given findById fails, when called, then returns the err", async () => {
  // given
  mockRepository.findById.mockResolvedValue(
    err(Err.code("unknown", { message: "Mongo database error" })),
  );
...
test("update: given repository update fails, when called, then returns the err", async () => {
  // given
  const existing = makeRun();
  mockRepository.findById.mockResolvedValue(ok({ data: existing }));
